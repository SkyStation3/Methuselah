...
#if defined(NDEBUG)
# error "Methuselah cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Methuselah Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache, bool fDryRun)//TODO--
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s SAP additional fees, %d delta bytes\n",
...
{
    CAmount nSubsidy = 10 * COIN; // 700801+

    // premine
    if (nHeight == 1)
        nSubsidy = 480000 * COIN;
    else if (nHeight >= 2 && nHeight <= 233600)
        nSubsidy = 75 * COIN;
    else if (nHeight >= 233601 && nHeight <= 467201)
        nSubsidy = 37.5 * COIN;
    else if (nHeight >= 467202 && nHeight <= 700801)
        nSubsidy = 18.75 * COIN;
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue, const Consensus::Params& consensusParams)
{
    ...
    bool bMasternodePayment = nHeight >= consensusParams.MasternodePaymentStartHeight; // 1921, 3 days
...
void ThreadScriptCheck() {
    RenameThread("methuselah-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if(pindex->nHeight > chainparams.GetConsensus().MasternodePaymentStartHeight + 7475)
    {
        ...
        else
        {
            ...
            CMethuselahAddress address(txdestaddr);
