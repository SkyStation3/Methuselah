...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "methuselahd compiled without wallet and -mineraddress not set");
#endif
    }
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated methuselah to.\n"
    ...
    CMethuselahAddress address(request.params[1].get_str());
    if (!address.IsValid())
...
// NOTE: Unlike wallet RPC (which use SAP values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "    https://github.com/methuselah/bips/blob/master/bip-0022.mediawiki\n"
            "    https://github.com/methuselah/bips/blob/master/bip-0023.mediawiki\n"
            "    https://github.com/methuselah/bips/blob/master/bip-0009.mediawiki#getblocktemplate_changes\n"
            "    https://github.com/methuselah/bips/blob/master/bip-0145.mediawiki\n"
...
//            "  },\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Methuselah is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Methuselah is downloading blocks...");
...
UniValue submitblock(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "See https://en.methuselah.it/wiki/BIP_0022 for full specification.\n"
...
UniValue estimatesmartfee(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) estimate fee-per-kilobyte (in SAP)\n"
...
UniValue masternodelist(const JSONRPCRequest& request)
{
             ...
             && strMode != "protocol" && strMode != "full" && strMode != "votes" && strMode != "donation" && strMode != "pose" && strMode != "common"))
    {
        throw runtime_error(
            ...
            "  votes          - Print all masternode votes for a Methuselah initiative (can be additionally filtered, partial match)\n"
        );
    }
...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in methuselah.conf\n"
            "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
            "  start-many   - Start all masternodes configured in masternode.conf\n"
            "  stop         - Stop masternode configured in methuselah.conf\n"
    ...
    if (strCommand == "genkey")
    {
        ...
        return CMethuselahSecret(secret).ToString();
    }
